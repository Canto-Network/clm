...
describe("Testing CNote exchange rate with redeems", async () => {
    ...
    it("Check that deployer redeems cNOTE for correct NOTE amount", async () => {
        const oldNoteBalance = (await note.balanceOf(dep.address)).toBigInt();
        const oldCNoteBalance = (await cNote.balanceOf(dep.address)).toBigInt();
        await cNote.connect(dep).redeem(oldCNoteBalance);
        const currentBorrows = (await cNote.callStatic.totalBorrowsCurrent()).toBigInt();
        const totalSupply = (await cNote.totalSupply()).toBigInt();
        const exchangeRate = currentBorrows * BigInt(1e18) / totalSupply;
        const newNoteBalance = (await note.balanceOf(dep.address)).toBigInt();
        const noteDifference = diff(oldNoteBalance, newNoteBalance);
        expect(exchangeRate).to.be.equal((await cNote.exchangeRateStored()).toBigInt());
        expect(noteDifference).to.be.equal((oldCNoteBalance * exchangeRate) / BigInt(1e18));
    })
    it("Check that user2 redeems cNOTE for correct NOTE amount", async () => {
        const currentBorrows1 = (await cNote.callStatic.totalBorrowsCurrent()).toBigInt();
        const totalSupply1 = (await cNote.totalSupply()).toBigInt();
        const exchangeRate1 = currentBorrows1 * BigInt(1e18) / totalSupply1;
        console.log(exchangeRate1);
        const oldNoteBalance = (await note.balanceOf(user2.address)).toBigInt();
        const oldCNoteBalance = (await cNote.balanceOf(user2.address)).toBigInt();
        await cNote.connect(user2).redeem(oldCNoteBalance);
        const currentBorrows = (await cNote.callStatic.totalBorrowsCurrent()).toBigInt();
        const totalSupply = (await cNote.totalSupply()).toBigInt();
        const exchangeRate = currentBorrows * BigInt(1e18) / totalSupply;
        const newNoteBalance = (await note.balanceOf(user2.address)).toBigInt();
        const noteDifference = diff(oldNoteBalance, newNoteBalance);
